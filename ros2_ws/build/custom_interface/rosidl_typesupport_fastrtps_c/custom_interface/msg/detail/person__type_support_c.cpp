...
#include "custom_interface/msg/detail/person__rosidl_typesupport_fastrtps_c.h"
...
using _Person__ros_msg_type = custom_interface__msg__Person;
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  {
    cdr << (ros_message->is_student ? true : false);
  }
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: is_student
  {
    uint8_t tmp;
    cdr >> tmp;
    ros_message->is_student = tmp ? true : false;
  }
...
size_t get_serialized_size_custom_interface__msg__Person(
  const void * untyped_ros_message,
  size_t current_alignment)
{
  ...
  // field.name is_student
  {
    size_t item_size = sizeof(ros_message->is_student);
    current_alignment += item_size +
      eprosima::fastcdr::Cdr::alignment(current_alignment, item_size);
  }
...
static uint32_t _Person__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_custom_interface__msg__Person(
      untyped_ros_message, 0));
}

ROSIDL_TYPESUPPORT_FASTRTPS_C_PUBLIC_custom_interface
size_t max_serialized_size_custom_interface__msg__Person(
  bool & full_bounded,
  bool & is_plain,
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = custom_interface__msg__Person;
    is_plain =
      (
      offsetof(DataType, is_student) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _Person__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_custom_interface__msg__Person(
...
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, custom_interface, msg, Person)() {
