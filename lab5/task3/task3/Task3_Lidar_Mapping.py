...
#
data = np.genfromtxt('task3/task3/Task3_AgoraRoom.csv', delimiter=',')[100:820]
...
room_map[40, 40] = 2  # This is the lidar (robot) position in the center

# Let's add some sample points to see how a map must look like
...
ax1.plot(pi * data[:, 0] / 180, data[:, 1], '.')
ax1.set_title("Polar Plot")

# Plot cartesian view
ax2 = fig.add_subplot(2, 2, 2)
ax2.plot(xs, ys, '.')
ax2.set_title("Cartesian Plot (x,y)")
ax2.axis('equal')

# Show the example map (as a matrix) with manually defining the cells' values
ax3 = fig.add_subplot(2, 2, 3)
ax3.imshow(room_map, cmap="gray")
ax3.set_title("Sample Room Map before bresenham")
#  
...
#
test_points = bresenham_points([40, 40], [10, 30]) + bresenham_points([40, 40], [10, 31])  ## NOTE: we can do + because they are lists, this is different with numpy arrays !!!!
print("Points in the bresenham line: {}".format(test_points))

# Now let's put these test points in the previous matrix.
...
ax4.set_title("Room Map with Bresenham Lines")
